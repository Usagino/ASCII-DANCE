...
export function main(_p5) {
  ...
  p5.setup = () => {
    ...
    movie = p5.createVideo('./movie/ballet5.mov') // eslint-disable-line
    movie.hide()
  ...
  p5.draw = () => {
    const balckColor = p5.color('#F8F8F8')
    p5.background(balckColor)
    const img = movie.get()
    const cols = fullSize.width / 8
    const rows = fullSize.height / 8
    ...
    for (let j = 0; j < rows; j++) {
      for (let i = 0; i < cols; i++) {
        ...
        const cx = x + d / 2
        const cy = y + d / 2
        const fillColor = img.get(cx, cy)
        const b = p5.sq(p5.brightness(fillColor) / 100)
